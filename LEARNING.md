...
| 단계 | 네트워크 | 이유 |
|------|---------|------|
| 개발/세팅 (지금) | 인터넷 연결 | 패키지 설치, 모델 다운로드 필요 |
| 운영 (나중) | 폐쇄망 (Host-only) | 보안, 데이터 외부 유출 방지 |

**지금은 개발 단계라 인터넷 연결 상태로 작업하는 것이 정상.**
모든 설치가 완료된 후 UTM 네트워크 설정을 Host-only로 변경하면 폐쇄망으로 전환됨.

### 폐쇄망 전환 시 체크리스트
- [ ] Ollama + Gemma 모델 설치 완료 확인
- [ ] Podman 이미지 모두 pull 완료 확인
- [ ] UTM 네트워크 → Host-only 변경
- [ ] VM간 내부 IP로 통신 확인
